...
check_environment() {
    ...
    # flutter_unified_repo 不存在时自动 clone flutter/flutter 并配置 .gclient
    if [ ! -d "$UNIFIED_REPO" ]; then
        log_info "flutter_unified_repo 不存在，正在克隆 Flutter SDK 仓库..."
...
# 列出可用的稳定版本
list_versions() {
    if [ ! -d "$UNIFIED_REPO/.git" ]; then
        log_warning "flutter_unified_repo 尚未初始化，请先运行: $0 <版本号>"
        return 0
    fi
...
switch_version() {
    ...
    # 检查 tag 是否存在，不存在则 fetch
    if ! git tag -l "$version" | grep -q "^${version}$"; then
        log_info "本地没有 tag '${version}'，正在 fetch..."
...
run_gclient_sync() {
    if [ ! -d "$DEPOT_TOOLS" ]; then
        ...
        log_info "请手动运行: export PATH=${DEPOT_TOOLS}:\$PATH && cd ${UNIFIED_REPO} && gclient sync"
        return 1
    fi
